    ...
    return user ? (
        <div className='relative h-full overflow-y-scroll bg-gray-50 p-6'>
            <div className='max-w-3xl mx-auto'>
                ...
                {/* Tabs */}
                <div className='mt-6'>
                    <div className='bg-white rounded-xl shadow p-1 max-w-md mx-auto'>
                                    ...
                                    <>
                                        {post.image_urls.map((image, index) => (
                                            <Link key={index} target='_blank' to={image} className='relative group'>
                                                <img src={image} alt="post-image" key={index} className='w-64 aspect-video object-cover'/>
                                                <p className='absolute bottom-0 right-0 text-xs p-1 px-3 backdrop-blur-xl text-white opacity-0 group-hover:opacity-100 transition duration-300'>Posted {moment(post.created_at).fromNow()}</p>
                                            </Link>
                                        ))}
                                    </>
                                ))
